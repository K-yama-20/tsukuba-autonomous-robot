...
def make_profile(upstream, mode, correction, *, device=None, host=None, udp=None, ptc=None, pcap=None):
    ...
    d.update(source_type={'hardware':1,'pcap':2,'packet_replay':3}[mode],use_gpu=False,
             use_timestamp_type=1,transform_flag=False,thread_num=2,channel_fov_filter_path='')
...
def main():
    ...
    config=make_profile(args.upstream,args.mode,args.correction,device=args.device_ip,host=args.host_ip,
                        udp=args.udp,ptc=args.ptc,pcap=args.pcap)
    import tempfile
    ...
    print('Receive-time fallback; firetime and mounting calibration remain unverified.')
