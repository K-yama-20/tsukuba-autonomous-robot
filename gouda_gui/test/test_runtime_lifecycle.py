...
class RuntimeLifecycleTest(unittest.TestCase):
    ...
    def test_nonfinite_or_negative_esp_age_fails_closed(self):
        state = {'mode': 'autonomy', 'processes': {'processing': {'pid': 1, 'start': 'owned'}}}
        for age in (float('nan'), float('inf'), -0.01):
            backend = {'recording': {'phase': 'idle'}, 'mapping': False,
                       'autonomy': {'profile_enabled': True, 'state_fresh': True,
                                    'state': {'phase': 'completed', 'goal': [1, 0, 0], 'device_fresh': True}},
                       'esp': {'auto_enabled': False}, 'ages': {'esp32': age}}
            with patch.object(runtime, 'alive', return_value=True), patch.object(runtime, 'urlopen', return_value=response(backend)):
                with self.assertRaisesRegex(RuntimeError, 'fresh ESP32 disarmed readback'):
                    runtime._ensure_disruption_safe(state)
    ...
    def test_runtime_status_returns_compact_backend_summary(self):
        snapshot = {'recording': {'phase': 'idle'}, 'mapping': False,
                    'autonomy': {'profile_enabled': False, 'state_fresh': False, 'state': None},
                    'esp': {'auto_enabled': False}, 'ages': {'esp32': .2},
                    'map': {'cells': [0] * 1000}, 'cloud': [list(range(100))], 'trail': [[1, 2]]}
        state = {'mode': 'observation', 'processes': {'processing': {'pid': 1, 'start': 'owned'}}}
        with tempfile.TemporaryDirectory() as directory, \
             patch.object(runtime, 'runtime_dir', return_value=Path(directory)), \
             patch.object(runtime, '_read_process_state', return_value=state), \
             patch.object(runtime, 'alive', return_value=True), \
             patch.object(runtime, 'urlopen', return_value=response(snapshot)):
            result = runtime.runtime_status()
        self.assertTrue(result['backend']['connected'])
        self.assertEqual(result['backend']['recording_phase'], 'idle')
        self.assertNotIn('state', result['backend'])
        self.assertNotIn('map', result['backend'])
        self.assertNotIn('cloud', result['backend'])
        self.assertNotIn('trail', result['backend'])
